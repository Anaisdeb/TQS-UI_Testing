...
//		Executor used to fix "WebDriverException: Element is not clickable at point (x, y)"
	...
	public void theUserEntersTheField(String way, String type, String from, String to, String departure, String back,
		...
		for (int i = adultsValue; i < Integer.parseInt(adults); i++) {
			driver.findElement(By.cssSelector(
					"#onereturn > div.col-lg-2 > div > div > div > div > div.dropdown-item.adult_qty > div > div > div.qtyInc"))
					.click();
		}
		;
		...
		for (int i = childsValue; i < Integer.parseInt(childs); i++) {
			driver.findElement(By.cssSelector(
					"#onereturn > div.col-lg-2 > div > div > div > div > div.dropdown-item.child_qty > div > div > div.qtyInc"))
					.click();
		}
		;
		...
		for (int i = infantsValue; i < Integer.parseInt(infants); i++) {
			executor.executeScript("arguments[0].click();", driver.findElement(By.cssSelector(
					"#onereturn > div.col-lg-2 > div > div > div > div > div.dropdown-item.infant_qty > div > div > div.qtyInc")));
		}
		;
	}
	...
	public void theResearchResultIs(String way, String type, String from, String to, String departure, String back,
			String adults, String childs, String infants) {
		...
		if (way.equals("one-way")) {
			headerExpected = String.format("%s %s\nDates ( %s )\nAdults %s Childs %s Infants %s\n Total Flights :",
					from, to, departure, adults, childs, infants);
			urlExpected = String.format("https://phptravels.net/flights/en/usd/%s/%s/oneway/%s/%s/%s/%s/%s", from, to,
					type, departure, adults, childs, infants).toLowerCase();
		} else {
			headerExpected = String.format("%s %s\nDates ( %s%s )\nAdults %s Childs %s Infants %s\n Total Flights :",
					from, to, departure, back, adults, childs, infants);
			urlExpected = String.format("https://phptravels.net/flights/en/usd/%s/%s/return/%s/%s/%s/%s/%s/%s", from,
					to, type, departure, back, adults, childs, infants).toLowerCase();
		}
